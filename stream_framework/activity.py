...
class Activity(BaseActivity):
    ...
    def __init__(self, actor, verb, object, object_type, target=None, target_type=None, time=None, extra_context=None):
        '''

        :param actor: should be able to handle a string or int value
        :param verb: provide a Verb object here
        :param object_id: Most content types accepted here will have timeuuid values
        :param target:
        :param time: datetime object?
        :param extra_context: Other values that we like to add to the object.
        :return:
        '''
        ...
        # Note that for actor and objects (and possibly target) string values will be accepted
        self._set_object_or_id('actor', actor)
        self._set_object_or_id('object', object)
        self._set_object_or_id('target', target)

        # Setting these to hard coded values for now
    ...
    def serialization_id(self):
        ...
        We might need to use timeuuids here to serialize?
        The returned serialization id will need to be a string
        '''

        # We should just return a timeuuid for the activity id based on the time value.
    ...
    def _set_object_or_id(self, field, object_):
        '''
        Either write the integer or string to
        field_id
        ...
        id_field = '%s_id' % field
        if isinstance(object_, (int, long, str, uuid.UUID, unicode)):  # Remember that we want to capture strings as ids
            setattr(self, id_field, object_)
        elif object_ is None:
            setattr(self, field, None)
            setattr(self, id_field, None)
        else:
            # raise Exception("An object outside of the int, long and str primitives has been supplied. Type is {0}".format(type(object_)))
            if isinstance(object_, uuid.UUID):
                setattr(self, field, str(object_))
                # The assumption is that we are supplying an object with an id field
                setattr(self, id_field, str(object_))
                # setattr(self, field, str(object_))
